...
import { FaGamepad, FaMoon, FaMusic, FaPlus, FaCog, FaHistory, FaTachometerAlt } from 'react-icons/fa';
...
const Sidebar = ({ collapsed, setCollapsed }) => {
  ...
  useEffect(() => {
    ...
    const fetchActiveModsCount = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/mods/active/count');
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4">
        ...
        {collapsed && <img src={logo} alt="ModHub Logo" className="h-8 w-8 mx-auto" />}
        <button 
      ...
      <div className="p-4 mb-2">
        <div className={`bg-green-800 rounded-lg p-2 ${collapsed ? 'text-center' : 'flex justify-between items-center'}`}>
          {collapsed ? (
      ...
      <div className="p-4">
        <div className={`text-gray-400 text-xs ${collapsed ? 'text-center' : ''}`}>
