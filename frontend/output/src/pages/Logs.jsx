...
import { FiRefreshCw, FiDownload, FiSearch, FiFilter, FiTrash2 } from 'react-icons/fi';
...
const Logs = () => {
  ...
  const exportLogsToCSV = () => {
    ...
    link.setAttribute('download', `modhub_logs_${format(new Date(), 'yyyy-MM-dd_HH-mm')}.csv`);
  ...
  const clearLogs = async () => {
    if (window.confirm('Are you sure you want to clear all logs? This action cannot be undone.')) {
      try {
        await axios.delete('http://localhost:8000/api/logs');
        setLogs([]);
        setFilteredLogs([]);
      } catch (err) {
                ...
                {filteredLogs.map((log, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${LOG_LEVELS[log.level]?.bgColor} ${LOG_LEVELS[log.level]?.color}`}>
                        {log.level}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
