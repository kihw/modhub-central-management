...
import { FaEdit, FaTrash, FaPause, FaPlay, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
const AutomationCard = ({ 
  ...
  const lastTriggered = automation.lastTriggered 
    ? formatDistance(new Date(automation.lastTriggered), new Date(), { addSuffix: true })
    : 'Never';
  
  const rulesMetCount = automation.conditions?.filter(condition => {
          ...
          <div className="p-4">
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <GiAutoRepair className={`text-xl mr-2 ${isActive ? 'text-blue-500' : 'text-gray-400'}`} />
              ...
              <div className="mt-1 flex justify-between">
                <div>
                  ...
                  <span className={`font-medium ${
                    rulesMetCount === 0 ? 'text-red-500' :
                    rulesMetCount === totalRules ? 'text-green-500' : 'text-yellow-500'
                  }`}>{rulesMetCount}/{totalRules}</span>
                </div>
                ...
                {automation.conditions && automation.conditions.length > 0 ? (
                  <ul className="space-y-1 text-sm">
                    {automation.conditions.map((condition, idx) => {
                      const isConditionMet = condition.type === 'process' && 
                        systemProcesses.some(proc => proc.name === condition.value);
                      ...
                      return (
                        ...
                        >
                          {condition.type === 'process' && (
                            <>Process <b>{condition.value}</b> is {condition.operation}</>
                          )}
                          {condition.type === 'time' && (
                            <>Time is {condition.operation} <b>{condition.value}</b></>
                          )}
                ...
                {automation.actions && automation.actions.length > 0 ? (
                  <ul className="space-y-1 text-sm">
                    {automation.actions.map((action, idx) => (
                      <li key={idx} className="py-1 px-2 rounded bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200">
                        {action.type === 'mod' && (
